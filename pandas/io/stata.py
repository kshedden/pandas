...
class StataReader(StataParser):
    ...
    def _read_new_header(self, first_char):
        ...
        # The rest of the header has version-specific struture.
        if self.format_version == 117:
            self._read_117_header()
        else:
            self._read_118_header()


    def _read_118_header(self):
        self.nobs = struct.unpack(self.byteorder + 'Q',
                                  self.path_or_buf.read(8))[0] # CHANGE
        self.path_or_buf.read(11)  # </N><label>
        strlen = struct.unpack(self.byteorder + 'H', self.path_or_buf.read(2))[0]
        self.data_label = self._decode(self.path_or_buf.read(strlen))
        self.path_or_buf.read(19)  # </label><timestamp>
        strlen = struct.unpack('b', self.path_or_buf.read(1))[0]
        self.time_stamp = self.path_or_buf.read(strlen).decode("utf-8")
        ...
        self.varlist = [self._null_terminate(self.path_or_buf.read(129))
                        for i in range(self.nvar)]
        ...
        self.fmtlist = [self._null_terminate(self.path_or_buf.read(57))
                        for i in range(self.nvar)]

        self.path_or_buf.seek(seek_value_label_names)
        self.lbllist = [self._null_terminate(self.path_or_buf.read(129))
                        for i in range(self.nvar)]

        self.path_or_buf.seek(seek_variable_labels)
        self.vlblist = [self._null_terminate(self.path_or_buf.read(321))
                        for i in range(self.nvar)]


    def _read_117_header(self):
    ...
    def _read_value_labels(self):
        ...
        while True:
            ...
            if self.format_version <= 117:
                ...
                labname = self._null_terminate(self.path_or_buf.read(129))
